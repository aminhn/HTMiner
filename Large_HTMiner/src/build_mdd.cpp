...
void Add_vec(vector<int>& items_lim, unordered_map<int, unsigned long long int>& ancest_map, unsigned long long int last_arc, int itmset);
...
void Add_vec(vector<int>& items_lim, unordered_map<int, unsigned long long int>& ancest_map, unsigned long long int last_arc, int itmset) {
	...
	if (Tree[last_arc].itmset > 0) {
		vector<unsigned long long int> ancest(L + 1, 0);							//last element of ancest is Carc child
		for (unordered_map<int, unsigned long long int>::iterator p = ancest_map.begin(); p != ancest_map.end(); ++p) {
