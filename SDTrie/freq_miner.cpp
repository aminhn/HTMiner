...
*/	
	...
	for (int pnt = 0; pnt < _patt.str_pnt.size(); ++pnt) {
		...
		while(!DFS_patt_init.empty()) { 
			int cur_arc = DFS_patt_init.back();
			DFS_patt_init.pop_back();
			for (vector<int>::iterator it2 = Tree[cur_arc].chld.begin(); it2 != Tree[cur_arc].chld.end(); ++it2) { 
				int cur_itm = Tree[*it2].item;
				if (cur_itm < 0) {
					cur_itm = -cur_itm;
					if (slist[cur_itm - 1]) {
						pot_patt[cur_itm + L - 1].freq += Tree[*it2].freq;
						pot_patt[cur_itm + L - 1].str_pnt.push_back(*it2);
					}
					if (Tree[*it2].chld.size() > 0) {
						DFS_patt.push_back(*it2);
					...
					if (ilist[cur_itm - 1]) {
						pot_patt[cur_itm - 1].freq += Tree[*it2].freq;
						pot_patt[cur_itm - 1].str_pnt.push_back(*it2);
					}
					if (Tree[*it2].chld.size() > 0) 
						DFS_patt_init.push_back(*it2);			
				}
			}
		}
		for (vector<int>::iterator it = _patt.ilist.begin(); it != _patt.ilist.end(); ++it)
		...
		while(!DFS_patt.empty()) {
			...
			for (vector<int>::iterator it2 = Tree[cur_arc].chld.begin(); it2 != Tree[cur_arc].chld.end(); ++it2) {
				int cur_itm = Tree[*it2].item;
				if (cur_itm > 0) {
					if (num_found == itmset_size && ilist[cur_itm - 1] && !check_parent(*it2, _patt.str_pnt[pnt], last_strpnt[cur_itm - 1], pot_patt[cur_itm - 1].str_pnt)) {
						pot_patt[cur_itm - 1].freq += Tree[*it2].freq;
						pot_patt[cur_itm - 1].str_pnt.push_back(*it2);
					}
					if (slist[cur_itm - 1] && Tree[Tree[*it2].anct].itmset <= Tree[_patt.str_pnt[pnt]].itmset) {
						pot_patt[cur_itm + L - 1].freq += Tree[*it2].freq;
						pot_patt[cur_itm + L - 1].str_pnt.push_back(*it2);
					}
					if (Tree[*it2].chld.size() > 0) {
