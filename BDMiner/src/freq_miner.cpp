	...
	for (int pnt = 0; pnt < _patt.str_pnt.size(); ++pnt) {
		...
		while(!DFS_patt.empty()) {
			...
			while (cur_sibl != -1) {
				int cur_itm = Tree[cur_sibl].item;
				if (cur_itm > 0) {
					if (num_found == itmset_size && ilist[cur_itm - 1] && (Tree[Tree[cur_sibl].anct].itmset < Tree[_patt.str_pnt[pnt]].itmset || !check_parent(cur_sibl, _patt.str_pnt[pnt], last_strpnt[cur_itm - 1], pot_patt[cur_itm - 1].str_pnt))) {
						pot_patt[cur_itm - 1].freq += Tree[cur_sibl].freq;
						pot_patt[cur_itm - 1].str_pnt.push_back(cur_sibl);
					}
