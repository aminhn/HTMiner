...
vector<unsigned int> DFS_seq;			//for sequence extension
...
void Freq_miner() {
	...
	DFS_itm.reserve(CTree.size());			
	DFS_seq.reserve(CTree.size());	
...
void Extend_patt(Pattern& _pattern) {
	...
	pot_patt.clear();
	for (int i = 0; i < L + L * ilist_nempty; ++i)
		pot_patt.emplace_back(_patt.str_pnt.size(), 1);
	if (!CTree.empty()) { 
		if (!VDFS.empty() && VDFS.back().ass_patt == DFS.size()) {
			pot_vpatt.clear();
			for (int i = 0; i < L + L * ilist_nempty; ++i)
				pot_vpatt.emplace_back(VDFS.back().str_pnt.size(), 1);
		}
		else
	...
	for (vector<int>::iterator it = ilistp.begin(); it != ilistp.end(); ++it) {
		...
		if (!CTree.empty() && !pot_vpatt[p].str_pnt.empty()) {
			...
			//pot_vpatt[p].str_pnt.shrink_to_fit();
			//pot_vpatt[p].seq_ID.shrink_to_fit();
			VDFS.emplace_back();
